...
from networks.local_fit.local_fit_model import TotalFLayer
...
def load_FLayer_and_cffLayer(model):
    LayerF = tf.keras.models.load_model(model, custom_objects = {
        'TotalFLayer': TotalFLayer})
...
# If use_specific_sets is False, search for all available kinematic sets in the folder
    ...
    available_kin_sets = [d for d in os.listdir('science/data/version_4/replicas/models') if d.startswith('kinematic_set_')]
...
for j in available_kin_sets:
    ...
    models_folder = f'science/data/version_4/replicas/models/kinematic_set_{str(j)}'
    ...
    cffs_pred_array = []
    cffs_stds_array = []
    # csv_path = os.path.join(scratch_path, f'CFFs_Predictions.csv')
    ...
    for i, cff_label in enumerate(cff_labels):
        ...
        all_cff_predictions_df.to_csv(f"CFF_Predictions/CFFs_Predictions_Set_{j}.csv", index=False)
        print(f"Predictions CSV saved: CFF_Predictions/CFFs_Predictions_Set_{j}.csv")
    ...
    def EvalResults(j, cffs_pred, cffs_std):
        pseudodata_df = {
            'set': [j],
    ...
    tempresults = EvalResults(j, cffs_pred_array, cffs_stds_array)
    tempresults.to_csv(f'Eval_set_{j}.csv', index=False)
...
df_kinematic = pd.read_csv("data.csv")
...
for set_id in df_cffs['set'].unique():
    ...
    # Repeat CFFs data for matching rows
    ...
    # Append combined data to the evaluation DataFrame
    df_evaluation = pd.concat([df_evaluation, df_combined], ignore_index=True)

# Ensure 'set' column is an integer
...
df_evaluation = df_evaluation.T.drop_duplicates().T

df_evaluation['set'] = df_evaluation['set'].astype(int)

# Save the evaluation DataFrame to a CSV file
df_evaluation.to_csv("DNN_projections_16_to_30.csv", index=False)