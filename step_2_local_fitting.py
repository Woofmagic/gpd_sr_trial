...
#Data file
initial_data_file = 'psueodata_with_sampling.csv'

#Number of sets
kinematic_sets = list(range(106, 111))
...
def gen_F_sanity_check(df_1, kinset, replica_id):
    ...
    create_folders(f'replica_cross_sections/kinematic_set_{kinset}')
    
    plt.savefig(f'replica_cross_sections/kinematic_set_{kinset}/sigma_vs_phi_kinematic_set_{kinset}_replica_{replica_id}.pdf')
    plt.close()
...
def run_replica(kinset, i, xdf):
    ...
    try:
        ...
        tfModel.save('DNNmodels_Kin_Set_' + str(kinset) + '/' + 'model' + str(replica_number) + '.h5', save_format='h5')
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('Losses_Plots_Kin_Set_' + str(kinset) + '/' + 'loss_plots_' + str(replica_number) + '.pdf')
        plt.close()

    except Exception as error:
        print(f"> Error running stuff: {error}")
...
for set_number in kinematic_sets:  # Loop through the list of kinematic sets
    ...
    create_folders(f'dnn_models_kinematic_set_{set_number}')
    create_folders(f'loss_plots_kinematic_set_{set_number}')

    # for replica_number in range(NUMBER_OF_REPLICAS):
    #     run_replica(set_number, replica_number, kin_df)
    ...
    tasks = [
        (set_number, replica_index, kin_df) for replica_index in range(NUMBER_OF_REPLICAS)
    ]

    number_of_processes =  5

    print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}]: Starting multithreading!")
    with mp.Pool(processes = number_of_processes) as pool:
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Started pool!")
        pool.starmap(run_replica, tasks)
        print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] FInished multithreading!", flush=True)
